...
save_archive <- function(archive, acq_function, acq_optimizer, metadata) {
  ...
  # Save the objects to files with timestamp
...
update_and_optimize <- function(acq_function, acq_optimizer, tmp_archive, candidate_new, lie, metadata) {
  print("11-here in update_and_optimize function:")
...
experiment <- function(data, metadata) {
  print("1-experiment_function started")
  set.seed(metadata$seed)
  print("2-data is loaded as below:")
  ...
  print("3-Loop through metadata$parameter_info to store input parameters for search space:")
  ...
  print("4-Loop through metadata$output_column_nameso to store output parameters for search space:")
  ...
  print("6- input and output values created from data and added to the archive, Model archive so far: ")
  print(archive)
  # Determine the number of objectives
  num_objectives <- length(metadata$output_column_names)
  print("7A- aquisition function and optimizer is created")
  ...
  print("7A- aquisition optimizer is created ")
  ...
  print("8- A list called result is created by using add_evals_to_archive function")
  
  result <- add_evals_to_archive(archive, acq_function,
                                 acq_optimizer, data, q, metadata)
  print("15A- result was created and archive is: ")
  ...
  print(archive)
  print("15B- acq_function$surrogate$archive is:")
  print(acq_function$surrogate$archive)
  
  x2 <- candidate[, names(metadata$parameter_info), with = FALSE]
  print("New candidates: ")
  print(x2)
  print("16- New archive: ")
  print(archive)
  ...
  print("Data no preds: ")
  print(data_no_preds)
  data.table::fwrite(data_no_preds, paste0(dir_path, "/output.csv"))
  print(paste0("mlr3mbo update is finished.
               Results file is saved at: ", dir_path))
  print("Data with preds: ")
  print(data_with_preds)
  data.table::fwrite(data_with_preds, paste0(dir_path, "/with-preds_output.csv"))
  print(paste0("mlr3mbo update is finished.
               Results file is saved at: ", dir_path))
...
metadata <- list(
  ...
  bucket_name = "Dual_Opt1",
  # user_id = "my_id",  # The user ID
  # table_name = "CuAlMnNi",  # The name of the table
  batch_number = "1",  # The batch number
  parameter_info = list(
    param1 = "float",  # The type of the Al parameter
    param2 = "float", # The type of the Ni parameter
    param3 = "float" # The type of the Ni parameter
    # Add more parameters as needed
  ),
  parameter_ranges = list(
    param1 = c(-5, 5),  # The range of the Al parameter
    param2 = c(-5, 5),  # The range of the Mn parameter
    param3 = c(-5, 5)  # The range of the Mn parameter
    # Add more ranges as needed
  ),
  output_column_names = c("target"),  # The names of the output columns
  ...
  directions = list(
    target = "minimize"
  ),  num_random_lines = 3,  # The number of random lines to generate
  to_nearest = 0.1  # The value to round to
)

#%%
# Run the experiment function
result <- experiment(data, metadata)
